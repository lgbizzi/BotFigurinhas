...
def build_remover_lote_handler(controller: BotController) -> ConversationHandler:
    """Construct and return the ConversationHandler for /remover_lote.
    ...
    async def cmd_remover_lote(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        """Entry point: prompt the user to send the list of codes to remove.
        ...
        """
        await update.message.reply_text(  # type: ignore[union-attr]
            tmpl.solicitar_lote_remover(),
            parse_mode=ParseMode.MARKDOWN,
        )
        return AGUARDANDO_LOTE

    async def receber_lote_remover(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        """Process the multi-line sticker list and end the conversation.
        ...
        resposta = await controller.processar_remover_lote(
            entradas_brutas=entradas,
            telegram_user_id=telegram_user_id,
            telegram_username=telegram_username,
        )
    ...
    return ConversationHandler(
        entry_points=[CommandHandler("remover_lote", cmd_remover_lote)],
        states={
            AGUARDANDO_LOTE: [
                CommandHandler("cancelar", cmd_cancelar),
                MessageHandler(filters.TEXT & ~filters.COMMAND, receber_lote_remover),
            ],
        },
        fallbacks=[CommandHandler("cancelar", cmd_cancelar)],
        name="remover_lote_conversation",
        persistent=False,
    )
